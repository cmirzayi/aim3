...
```{r}
yass
colnames(tax_table(yass)) <- c("kingdom", "phylum", "class", "order", "family", "genus", "species")
...
```{r}
mod1 <- lmer(value ~ penicillins + macrolides + cephalosporins + sulfonamides + other + bs(infant_age) + (1|subject_id), data=yassbact)
summary(mod1)
summary(mod1)
library(lmerTest)
anova(mod1)
...
taxon_glm <- function(taxon) {
  ...
  fit <- tryCatch((broom::tidy(modfit)), error=function(e) NULL)
    if (dim(broom::tidy(modfit))[2] > 0) {
