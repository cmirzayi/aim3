...
pcaplot1 <- ggordpoint(obj=yasspca, biplot=TRUE,
                      factorNames=c("abx"), ellipse=TRUE) + theme(legend.position = "none")
pcaplot2 <- ggordpoint(obj=yasspca, pc=c(1, 3), biplot=TRUE,
                       factorNames=c("abx"), ellipse=TRUE)
pcaplot1 | pcaplot2
```
...
sY <- sY %>% dplyr::arrange(subject_id, infant_age) %>% group_by(subject_id) %>% mutate(prevShannon=lag(Shannon), prevPC1=lag(PC1), prevPC2=lag(PC2))
...
sYreduc <- sY %>% select(sample_id, prevShannon, prevPC1, prevPC2)
...
# Summarise
library(kableExtra)
pen <- cbind(topTable(fit, coef=2, p.value=0.05, number=1000), oldname=rownames(topTable(fit, coef=2, p.value=0.05, number=1000)))

inner_join(yasstax, pen) %>% View()
```
...
vesuvius(x=13)+ggtitle("Penicillins x lagged Shannon")
vesuvius(x=14)+ggtitle("Macrolides x lagged Shannon")
vesuvius(x=15)+ggtitle("Cephalosporins x lagged Shannon")
vesuvius(x=16)+ggtitle("Sulfonamides x lagged Shannon")

vesuvius(x=17)+ggtitle("Penicillins x lagged PC1")
vesuvius(x=18)+ggtitle("Macrolides x lagged PC1")
vesuvius(x=19)+ggtitle("Cephalosporins x lagged PC1")
vesuvius(x=20)+ggtitle("Sulfonamides x lagged PC1")

vesuvius(x=21)+ggtitle("Penicillins x lagged PC2")
vesuvius(x=22)+ggtitle("Macrolides x lagged PC2")
vesuvius(x=23)+ggtitle("Cephalosporins x lagged PC2")
vesuvius(x=24)+ggtitle("Sulfonamides x lagged PC2")
```
...
```{r}
# Interaction model between commonly coprescribed abx
```
...
dfyass <- inner_join(dfyass, shannon)
...
table1(~ `Assessments Completed` + `# of Antibiotic Courses` + `Birth Year` + Gender + `Delivery Mode` + `Shannon Alpha Diversity`| `Antibiotics Exposure`, data=participants) %>% t1flex() %>% save_as_docx(path="table1.docx")
```
